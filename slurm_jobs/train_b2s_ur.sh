#!/bin/bash
#SBATCH --job-name="diffwave-brain-finetuning-hp"
#SBATCH --nodes=1
#SBATCH --ntasks=72
#SBATCH --gpus=4
...
echo
echo FINETUNE UNCONDITIONAL DIFFWAVE MODEL ON HARRY POTTER ECOG AND AUDIO DATA
echo 
echo $(date +"%D %T")
echo
...
python src/train.py \
    ...
    model.pretrained_generator=exp/Uncond-PT-v9/checkpoint/230.pkl \
    train.n_epochs=200 \
