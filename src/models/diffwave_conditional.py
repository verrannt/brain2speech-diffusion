...
class DiffWaveConditional(nn.Module):
    ...
    def __init__(
        self, encoder_class: nn.Module, encoder_cfg: DictConfig, decoder_cfg: DictConfig
    ) -> None:
        super().__init__()
    ...
    def load_pretrained_generator(
        ...
        inc_keys = self.speech_generator.load_state_dict(
            checkpoint_dict["model_state_dict"], strict=False
        )

        # In any experiment setting, there should never be keys we don't expect
        assert (
            len(inc_keys.unexpected_keys) == 0
        ...
        assert all(
            ["local_conditioner" in k for k in inc_keys.missing_keys]
        ...
        if self.encoder.__class__.__name__ == "BrainClassEncoder":
            ...
            self.encoder.class_conditioner.load_state_dict(
                checkpoint_dict["conditioner_state_dict"], strict=True
            )
