...
def main(args):
    ...
    model_cfg = OmegaConf.load(
        Path.home() / "brain2speech-diffusion/configs/model/brain_class_conditional.yaml"
    )
    ...
    for k in clf_ckpt["model_state_dict"].keys():
        assert (
            final_ckpt["conditioner_state_dict"][f"brain_classifier.{k}"]
            == clf_ckpt["model_state_dict"][k]
        ).all()
    ...
    for k in pret_ckpt["conditioner_state_dict"]:
        ...
        assert (
            final_ckpt["conditioner_state_dict"][f"class_conditioner.{k}"]
            == pret_ckpt["conditioner_state_dict"][k]
        ).all()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Merge pretrained classconditional model with brain classifier"
    )
    parser.add_argument(
        "--pretrained", type=str, help="Checkpoint of the class conditional pretraining model"
    )
