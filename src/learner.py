...
class Learner:
    ...
    def train(self) -> None:
        ...
        for epoch in range(start_epoch, self.n_epochs + start_epoch):
            ...
            for i, data in enumerate(
                tqdm(trainloader, desc="Training", ncols=100, disable=not self.is_master)
            ):
    ...
    def unpack_input(
        self, data: Tuple[Tensor, int, Union[Tensor, str], str]
    ...
    def load_checkpoint(self, checkpoint_directory):
        ...
        if self.ckpt_epoch >= 0:
            try:
                ...
                print(
                    f"Model checkpoint found at epoch {self.ckpt_epoch}, but was not successfully loaded - training from scratch."
                )
                self.ckpt_epoch = -1
        else:
    ...
    def load_pretrained_generator(self):
        ...
        if self.model_cfg.get("pretrained_generator", False):
            if self.model_cfg.unconditional:
                print(
                    "Pretrained generator assigned, but will not be loaded since this is an unconditional model."
                )
            else:
                if self.ckpt_epoch != -1:
                    ...
                    if self.model_cfg.get("freeze_generator", False):
                        checkpoint = torch.load(
                            self.model_cfg.pretrained_generator, map_location="cpu"
                        )
                        self.model.load_pretrained_generator(checkpoint, freeze=True)
                    else:
                        print(
                            "Pretrained generator assigned, but will not be loaded since model has been loaded "
    ...
    def generate_samples(self, epoch: int) -> None:
        ...
        samples = [
            wandb.Audio(sample.squeeze().cpu(), sample_rate=self.dataset_cfg.sampling_rate)
    ...
    def compute_loss(self, audio: Tensor, conditional_input: Optional[Tensor] = None):
        ...
        epsilon_theta = self.model(
            (transformed_X, diffusion_steps.view(B, 1)), conditional_input=conditional_input
        )
