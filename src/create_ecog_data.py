...
def main(
    ...
    for _run in [1,2]:
        ...
        textgrid = TextGrid(
            data_dir / f'{subject_id}_ses-iemu_acq-ECOG_run-0{_run}_audio.TextGrid')
        ...
        ecog_file_path = data_dir / f'{subject_id}_ses-iemu_acq-ECOG_run-0{_run}_ieeg/{hfb_filename}'
        ecogs.append(np.load(ecog_file_path))

        audio_file_path = data_dir / f'{subject_id}_ses-iemu_acq-ECOG_run-0{_run}_audio_pitch_shifted.wav'
    ...
    max_len = round(np.max([interval.xmax - interval.xmin for interval in intervals]) * sampling_rate)
    ...
    for interval in tqdm(intervals, desc="Processing Intervals"):
        ...
        ecog = ecogs[interval.run-1][            
            int(interval.xmin * sampling_rate) : 
            int(interval.xmin * sampling_rate) + max_len
