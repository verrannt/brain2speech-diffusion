...
def main(
    ...
    for fn in track(os.listdir(output_path), description="Fixing lengths", transient=True):
    ...
    console.log(
        f"Unified all files to length of {segment_length} samples at " f"{sampling_rate} Hz"
    )
    ...
    for word in targets:
        ...
        chosen_speaker_ids = rng.choice(speaker_ids, n_validation_speakers, replace=False)
        # Pick files
        for id in chosen_speaker_ids:
            matching_files = [fn for fn in train_files if id in fn and word in fn]
    ...
    console.log(f"Saved training and validation splits at {splits_output_path}")
